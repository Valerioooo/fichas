...
O servidor Xampp não habilita os \textit{Sockets} por defeito, sendo necessário habilita-los por meio da edição do ficheiro php.ini se o Sistema Operativo utilizado for \textit{Windows}, e a compilação do código fonte da linguagem PHP com a \textit{flag} \say{--enable-sockets} caso o Sistema Operativo utilizado for MacOS ou Linux.

A Prof. Ester Tavares ajudou-me bastante na execução desta ficha e também no desenvolvimento do Projeto Final de curso.

Na minha opinião a parte mais interessante do curso foi o Projeto Final, pois proporcionou-me descobrir a ferramenta {\textbf{\textit{Bootstrap}} que se revelou uma grande ajuda ao desenvolvimento do Site.
